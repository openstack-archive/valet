...
class ValetFilter(filters.BaseHostFilter):
    ...
    def __init__(self):
        ...
        self.failure_mode = opt[self.opt_failure_mode_str]
    ...
    def _do_adhoc_placement(self, res_id, hosts, availability_zone,
            instance_type):
        """Ad-hoc placements
    ...
    def filter_all(self, filter_obj_list, filter_properties):
        ...
        if failed:
            LOG.warn("Failed to filter the hosts,"
                     " failure mode is %s" % self.failure_mode)
            if self.failure_mode == 'yield':
                LOG.warn(failed)
                yield_all = True
            else:
        ...
        # if not filter_properties.get(hints_key, {}).has_key(orch_id_key):
            ...
            self._do_adhoc_placement(res_id, hosts, availability_zone,
                    instance_type)
        else:
            ...
            if location:
                ...
                if match:
                    if self._ad_hoc:
                        ...
                        LOG.info(
                            _LI("Valet placement for resource id %s,"
