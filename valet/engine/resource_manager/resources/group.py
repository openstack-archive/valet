...
class Group(object):
    ...
    def __init__(self, _name):
        ...
        self.vms_per_host = {}           # key = host_name, value = a list of placed vms
    ...
    def add_vm(self, _vm_info, _host_name):
        ...
        if self.exist_vm_in_host(_host_name, orch_id=_vm_info["orch_id"], uuid=_vm_info["uuid"]):
            self.remove_vm_from_host(_host_name, orch_id=_vm_info["orch_id"], uuid=_vm_info["uuid"])
    ...
    def remove_vm_from_host(self, _host_name, orch_id=None, uuid=None):
        ...
        if self.group_type == "EX" or self.group_type == "AFF" or self.group_type == "DIV":
            if (_host_name in self.vms_per_host.keys()) and len(self.vms_per_host[_host_name]) == 0:
