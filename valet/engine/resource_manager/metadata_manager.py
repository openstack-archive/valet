...
class MetadataManager(threading.Thread):
    ...
    def _run(self):
        ...
        if self.set_groups() is not True:
            self.logger.warn("fail to set groups (availability zones and host-aggregates) from nova")
    ...
    def set_groups(self):
        ...
        compute = NovaCompute(self.logger)
            ...
            for hk in lg.vms_per_host.keys():
                if hk in self.resource.hosts.keys():
                    ...
                    if lgk not in rhost.memberships.keys():
                        ...
                        self.logger.info("host (" + rhost.name + ") updated (new membership)")
                ...
                for mk in rhost.memberships.keys():
                    ...
                    if m.group_type != "EX" and m.group_type != "AFF" and m.group_type != "DIV":
                        if mk not in _groups.keys():
                            ...
                            membership_updated = True
                            self.logger.info("host (" + rhost.name + ") updated (delete membership)")
                        else:
                            lg = _groups[mk]
    ...
    def _check_flavor_update(self, _flavors):
        ...
        for fk in _flavors.keys():
            if fk not in self.resource.flavors.keys():
                ...
                self.logger.info("new flavor (" + fk + ":" + _flavors[fk].flavor_id + ") added")
