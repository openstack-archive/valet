...
class AppHandler(object):
    ...
    def set_app(self, _app):
        ...
        LOG.info("Received '" + app_topology.action + "' for app = " +
                 app_topology.app_id)
    ...
    def _set_app_for_ad_hoc_update(self, _placement, _app_topology):
        ...
        if _placement.stack_id is None or _placement.stack_id == "none":
            ...
            if old_placements is None:
                ...
                _app_topology.status = "missing prior stack while ad-hoc"
                                       " update"
                return
        ...
        if flavor_id is None:
            _app_topology.status = "missing vm flavor info  while ad-hoc"
                                   " update"
            return
                    ...
                    if rk in _app_topology.stack["placements"].keys():
                        if placement.stack_id is None or \
                           ...
                           placement.stack_id != _app_topology.app_id:
                            ...
                            self.phandler.update_placement(
                                    uuid, stack_id=_app_topology.app_id,
                                    orch_id=rk, state=curr_state)

                        self._apply_meta_change(rk, r,
                                _app_topology.stack["placements"])
    ...
    def _change_orch_id(self, _app, _app_topology):
        ...
        if host_name == "none":
            _app_topology.status = "allocated host is not found while changing"
                                   " mock id"
            LOG.error(_app_topology.status)
            return _app_topology.status
        else:
            if host_name in self.resource.hosts.keys():
                ...
                if vm_info is None:
                    _app_topology.status = "vm is not found while changing "
                                           "mock id"
                    LOG.error(_app_topology.status)
                    return _app_topology.status
                else:
    ...
    def delete_from_stack(self, _stack_id, orch_id=None, uuid=None):
        ...
        if not self.db.delete_placement_from_stack(_stack_id, orch_id=orch_id,
