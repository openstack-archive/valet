...
class Analyzer(object):
    ...
    def init_instances_for_group(self, resources):
        ...
        for grp in resources.groups.keys():
            ...
            ins_group[group_types[index]].append(resources.groups[grp].group_resources)
            ++index
    ...
    def are_we_alone(self, ins_for_group, group_name, level):
        ...
        # Get the hosts which are involved in the exclusivity group.  There could
        # be potentially many if group is at the rack level, or only one if at the
        # host level.
    ...
    def get_exclusivity_group_hosts(self, group_name):
        """ Get all the hosts that the exclusivity group instances are
        located on.
