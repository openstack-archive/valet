...
class TestAppTopology(Base):
    ...
    def test_set_app_topology(self, mock_set_topology):
        self.app_topology.parser = Parser("test_high_level")

        test_vg = VGroup("app_uuid", "uuid")
        test_vm = VM("vm_app", "vm_uuid")
        ...
        mock_set_topology.return_value = (vgroups, vms)

        self.app_topology.set_app_topology("app_graph")
        self.assertEqual(test_vg, self.app_topology.vgroups[test_vg.uuid])
        self.assertEqual(test_vm, self.app_topology.vms[test_vm.uuid])
    ...
    def test_set_vgroup_weight(self):
        self.app_topology.resource.CPU_avail = 1.0
        self.app_topology.resource.mem_avail = 2.0
        self.app_topology.resource.local_disk_avail = 3.0

        vgroup = VGroup("mock_app", "mock_uuid")
        vgroup.vCPUs = 3.0
        vgroup.mem = 8.0
        vgroup.local_volume_size = 6.0

        self.app_topology._set_vgroup_weight(vgroup)

        self.assertEqual(vgroup.vCPUs / self.app_topology.resource.CPU_avail,
                         vgroup.vCPU_weight)
        self.assertEqual(vgroup.mem / self.app_topology.resource.mem_avail,
                         vgroup.mem_weight)
        self.assertEqual(6.0 / 3.0, vgroup.local_volume_weight)

    def test_set_optimization_priority_cpu(self):
        self.app_topology.vgroups = {
            "foo": "bar"
        }
        self.app_topology.vms = {
    ...
    def test_set_optimization_priority_lvol(self):
        self.app_topology.vgroups = {
            "foo": "bar"
        }
        self.app_topology.vms = {
        ...
        self.app_topology.resource.CPU_avail = 0
        self.app_topology.total_CPU = 10

        self.app_topology.resource.mem_avail = 0
        self.app_topology.total_mem = 0
