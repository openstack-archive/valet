...
class CheckForAssertingNoneEquality(BaseASTChecker):
        ...
        MSG_IS = ('V001 Use self.assertIsNone(...) when comparing '
                  'against None')
        MSG_IS_NOT = ('V002 Use self.assertIsNotNone(...) when comparing '
                      'against None')
            ...
            if isinstance(node.func, ast.Attribute):
                if node.func.attr == "assertEqual":
                    for arg in node.args:
                        if _is_None(arg):
                            self.add_error(node, message=self.MSG_IS)
                elif node.func.attr == "assertNotEqual":
                    for arg in node.args:
                        if _is_None(arg):
                            self.add_error(node, message=self.MSG_IS_NOT)
