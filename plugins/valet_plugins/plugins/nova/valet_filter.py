...
#         if not filter_properties.get(hints_key, {}).has_key(orch_id_key):
            ...
            while count < self.retries:
                ...
                if response is None:
                    count += 1
                    msg = ("Valet conn error for plan_create Retry %s "
                           "for stack = %s")
                    LOG.warn(msg % (str(count), res_id))
            ...
            if not location:
                msg = _LE("Valet ad-hoc placement unknown for resource id %s.")
                LOG.error(msg % res_id)
                if failure_mode == 'yield':
            ...
            while count < self.retries:
                ...
                if response is None:
                    count += 1
                    msg = _LW("Valet conn error for placement Retry %s "
                              "for stack = %s")
                    LOG.warn(msg % (str(count), orch_id))
                    time.sleep(self.interval)
                else:
                    break
            ...
            if location:
                ...
                if match:
                    if ad_hoc:
                        ...
                        msg = _LI("Valet placement for resource id %s, "
