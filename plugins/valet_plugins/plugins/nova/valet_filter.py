...
from valet_plugins.i18n import _LE
from valet_plugins.i18n import _LI
from valet_plugins.i18n import _LW
...
class ValetFilter(filters.BaseHostFilter):
    ...
    def _location_for_resource(self, hosts, filter_properties):
        ...
        LOG.info(_LI("Resolving Orchestration ID "
        ...
        if orch_id:
            ...
            if action == 'replan':
                message = _LI("Forcing replan for Resource ID: {}, "
                              "Orchestration ID: {}")
            else:
                message = _LI("Reserving with possible replan "
                              "for Resource ID: {}, "
                              "Orchestration ID: {}")
            ...
            LOG.info(_LW("Orchestration ID not found. "
    ...
    def _orch_id_from_resource_id(self, res_id):
        ...
        LOG.info(_LI("Searching by Resource ID: {}.").format(res_id))
    ...
    def _orch_id_from_scheduler_hints(self, hints={}):
        ...
        if all(k in hints for k in (root_stack_id_key, resource_name_key,
            ...
            LOG.info(_LI("Searching placements via scheduler hints."))
            response = self.api.placements_search(**kwargs)
            orch_id = \
                self._first_orch_id_from_placements_search(response)
            return orch_id
    ...
    def filter_all(self, filter_obj_list, filter_properties):
        ...
        try:
            ...
            LOG.error(_LE('Authorization exception: {}').format(err.message))
        ...
        if authorized:
            ...
            try:
                ...
                LOG.error(_LE('API Exception: {}').format(err.message))

        # Honk if we didn't find a location and decide if we yield to Nova.
        if not location:
            if ad_hoc:
                message = _LE("Ad-hoc placement unknown, "
                              "Resource ID: {}")
                LOG.error(message.format(res_id))
            elif orch_id:
                message = _LE("Placement unknown, Resource ID: {}, "
                              "Orchestration ID: {}.")
                ...
                message = _LE("Placement unknown.")
                LOG.error(message)

            if CONF.valet.failure_mode == 'yield':
                message = _LW("Yielding to Nova or placement decisions.")
                LOG.warn(message)
                yield_all = True
            else:
                message = _LE("Rejecting all Nova placement decisions.")
                LOG.error(message)
                yield_all = False
        ...
        for obj in filter_obj_list:
            if location:
                ...
                if match:
                    if ad_hoc:
                        message = _LI("Ad-Hoc host selection "
                                      "is {} for Resource ID {}.")
                        LOG.info(message.format(obj.host, res_id))
                    else:
                        message = _LI("Host selection is {} for "
                                      "Resource ID: {}, "
                                      "Orchestration ID: {}.")
