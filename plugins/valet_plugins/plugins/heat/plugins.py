...
class ValetLifecyclePlugin(lifecycle_plugin.LifecyclePlugin):
        ...
        for name, resource in stack.resources.iteritems():
            ...
            if not key:
                ...
                if not key:
                    ...
                    key = str(uuid.uuid4())
                    LOG.warn(_LW("Orchestration ID not found for resource "
            ...
            LOG.info(_LI("Adding Resource, Type: {}, "
                     "Name: {}, UUID: {}").format(
    ...
    def do_pre_op(self, cnxt, stack, current_stack=None, action=None):
        ...
        if not self.hints_enabled:
            LOG.warn(_LW("stack_scheduler_hints is False, skipping stack"))
            return
        ...
        if action == 'DELETE' and stack and stack.status == 'IN_PROGRESS':
            ...
            LOG.info(_LI('Requesting plan delete for '
            ...
            try:
                ...
                LOG.warn(_LW("Plan not found, proceeding with stack delete."))
        elif action == 'UPDATE' and stack \
                and current_stack and current_stack.status == 'IN_PROGRESS':
            LOG.info(_LI('Building plan update request for '
                         'stack "{}" ({})').format(stack.name, stack.id))
            ...
            try:
                ...
                LOG.warn(_LW("Plan not found, creating a new plan"))
                plan = {
            ...
            LOG.info(_LI('Building plan create request for '
                         'stack "{}" ({})').format(stack.name, stack.id))

            resources = self._parse_stack(stack)
            if not resources:
                LOG.warn(_LW("No resources found, skipping stack"))
                return
