...
class ValetAPI(object):
    ...
    def _exception(self, exc_info, response):
        ...
        try:
            ...
            # Note: msg components use i18n instead, don't use _LE 2x
            msg = "{} (valet-api: {})".format(
                response_dict.get('explanation', _LE('Unknown remediation')),
                error.get('message', _LE('Unknown error')))
    ...
    def _request(self, method='get', content_type='application/json',
        ...
        for attempt in range(retries):
            if attempt > 0:
                LOG.warn(_LW("Retry #{}/{}").format(attempt + 1, retries))
            try:
